...
export class CreatedPlaylists extends Component {
  state = {
    playlists: [],
    loading: true,
  };
  ...
  fetchPlaylists = async () => {
    const { createdPlaylists } = this.props;

    try {
      const playlistRequests = createdPlaylists.map(id =>
  ...
  render() {
    ...
    if (loading) {
      return <p>Playlists loading...</p>;
    }

    return (
      <div className="created-playlists" style={styles.container}>
        <h2 style={styles.header}>{userName}'s Playlists</h2>
        {playlists.length > 0 ? (
